...
EnemyAimState::EnemyAimState()
{
	...
	timer.set_on_timeout(
		[&]()
		{
			Enemy* enemy = static_cast<Enemy*>(CharacterManager::instance()->get_enemy());
...
void EnemyDashInAirState::on_enter()
{
	...
	Vec2 pos_target = { player->get_position().x, player->get_floor_y() };
...
void EnemyIdleState::on_enter()
{
	...
	if (enemy->get_hp() > 7)
		wait_time = randint(0, 2) * 0.25f; // 0.0 ~ 0.50s
	else
		wait_time = randint(0, 1) * 0.25f; // 0.0 ~ 0.25s
...
void EnemyRunState::on_update(float delta)
{
	...
	const Vec2& pos_enemy = enemy->get_position();
	const Vec2& pos_player = CharacterManager::instance()->get_player()->get_position();
...
EnemyThrowSilkState::EnemyThrowSilkState()
{
	...
	timer.set_on_timeout(
		[&]()
		{
			...
			if (!enemy->is_on_floor() && enemy->get_hp() > 7 && randint(1, 100) <= 25)
